...
from untils.extract_jsonstr_until import extract_jsonstr

df = pd.read_excel("../data/sql_seed_result.xlsx")
df['predict_result'] = ''
...
for i in range(len(df)):
    ...
    try:
        ...
        result = e
        df['predict_result'][i] = result
    if df['result'][i] == df['predict_result'][i]:
        count += 1

print(df)
df['acc'] = count/len(df)
df.to_excel('../data/sql_generate_result.xlsx', index=False, engine='openpyxl')
