...
from test.rag_bak.A_testRAG import ChineseTextSplitter,zh_title_enhance
...
def load_file(filepath, sentence_size=SENTENCE_SIZE, using_zh_title_enhance=ZH_TITLE_ENHANCE):
    ...
    if filepath.lower().endswith(".md"):
        ...
        from test.rag_bak.A_testRAG import UnstructuredPaddlePDFLoader
        loader = UnstructuredPaddlePDFLoader(filepath)
        textsplitter = ChineseTextSplitter(pdf=True, sentence_size=sentence_size)
        docs = loader.load_and_split(textsplitter)
    elif filepath.lower().endswith(".jpg") or filepath.lower().endswith(".png"):
        # 暂且将paddle相关的loader改为动态加载，可以在不上传pdf/image知识文件的前提下使用protobuf=4.x
        from test.rag_bak.A_testRAG import UnstructuredPaddleImageLoader
